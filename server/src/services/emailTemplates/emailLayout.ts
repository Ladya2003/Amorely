...
export type TransactionalEmailHtmlContent = {
  htmlLang: string;
  title: string;
  heading: string;
  subtitle: string;
 ...
 */
export const buildTransactionalEmailHtml = (content: TransactionalEmailHtmlContent): string => {
  ...
  const linkHint = escapeHtml(content.linkHint);
  const footerNote = escapeHtml(content.footerNote);
  const madeForCouples = escapeHtml(content.madeForCouples);
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:520px;">
          ...
          <tr>
            <td align="center" style="padding:22px 8px 0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;font-size:12px;line-height:1.5;color:${MUTED};">
              ${madeForCouples} · <a href="https://amorely.love" style="color:${BRAND};text-decoration:none;">amorely.love</a>
            </td>
          </tr>
        </table>
      </td>
